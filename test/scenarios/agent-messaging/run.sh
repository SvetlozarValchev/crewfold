...
"$binary" agent create requester --workspace personal --role implementer --provider fixture-mcp --runtime direct --socket "$socket_path" --idempotency-key mailbox-requester-agent --output json >"$scenario_root/requester-agent.json"
"$binary" agent create reviewer --workspace personal --role reviewer --provider fixture-mcp --runtime direct --socket "$socket_path" --idempotency-key mailbox-reviewer-agent --output json >"$scenario_root/reviewer-agent.json"
...
"$binary" run start "$requester_task" --workspace personal --runtime direct --provider fixture-mcp --scenario "$repo_root/test/fixtures/agent-messaging/requester.json" --expected-task-revision 2 --socket "$socket_path" --idempotency-key mailbox-requester-run --output json >"$scenario_root/requester-run.json"
