...
temp_dir=$(mktemp -d "${TMPDIR:-/tmp}/crewfold-build.XXXXXX")
...
$go_runner build -trimpath \
  -ldflags '-X crewfold/internal/buildinfo.version=0.0.0-test -X crewfold/internal/buildinfo.commit=acceptance-commit -X crewfold/internal/buildinfo.builtAt=2026-08-12T00:00:00Z' \
  -o "$release_binary" ./cmd/crewfold
"$release_binary" version --output json >"$temp_dir/release-version.json"
grep -Fq '"version":"0.0.0-test"' "$temp_dir/release-version.json"
...
printf 'Buildable repository acceptance: PASS\n'
