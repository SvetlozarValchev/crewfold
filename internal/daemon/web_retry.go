...
func (w *workbenchServer) handleWorkbenchRunRetry(response http.ResponseWriter, request *http.Request, session workbenchSession) {
	...
	if err := decoder.Decode(&params); err != nil || decodeHasTrailingValue(decoder) || strings.TrimSpace(params.Workspace) == "" || !validCanonicalEntityID(params.Run, "run_") || !validWorkbenchText(params.IdempotencyKey, 128) {
		w.writeError(response, http.StatusBadRequest, "invalid_request", "run retry requires exact workspace, failed run, and idempotency key")
		return
	}
	...
	if prior.Run.Status != domain.RunStartFailed {
		w.writeStoreError(response, &store.Error{Code: store.CodeRunConflict, Message: "only an exact start_failed run can be retried from the workbench"})
		return
	}
	...
	result, err := w.daemon.store.CreateRun(request.Context(), store.CreateRunCommand{
		...
		IdempotencyKey: params.IdempotencyKey, CorrelationID: "web-retry-" + hex.EncodeToString(digest[:12]),
	})
	if err != nil {
		w.writeStoreError(response, err)
		return
