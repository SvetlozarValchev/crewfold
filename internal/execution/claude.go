...
func claudeInitialPrompt(requiredEvidence []string) string {
	...
	return "You are a Crewfold-managed Claude Code worker. First call the Crewfold MCP tool crewfold_get_briefing and treat that immutable briefing as the authority for your role, task, checkout, policies, durable inbox, and reporting contract. Work only on the assigned task in the current checkout. Use Crewfold MCP for progress, durable messages, artifacts, blockage, and completion; do not substitute final terminal text or provider session state for a Crewfold report. Read any applicable provider handoff from the Crewfold inbox, not another provider's transcript. If you cannot proceed, call crewfold_report_blocked and then end the run. Before completion, inspect the diff and run the task's checks, then call crewfold_propose_completion with an executive handoff. Required completion evidence IDs: " + evidence + ". Never push, publish, use network tools, or access unrelated projects."
}
