...
func TestM22ReadableCodexItemCoversObservableWorkWithoutPrivateReasoning(t *testing.T) {
	tests := []struct {
		...
		{name: "accepted task", raw: `{"id":"task-1","type":"userMessage","clientId":"crewfold-run:run_0123456789abcdef0123456789abcdef","content":[{"type":"text","text":"Crewfold attached one accepted task turn"}]}`, want: domain.DomainAgentSessionItem{ID: "task-1", Type: "userMessage", Origin: "crewfold_task", Text: "Crewfold attached one accepted task turn"}, wantOK: true},
