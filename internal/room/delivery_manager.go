...
Do not poll or wait in a terminal; Crewfold will deliver later room activity to this same Codex thread.`, route.Participant.Handle, route.Room.Title, route.Room.Slug, strings.Join(lines, "\n\n"), command, route.Room.Slug, route.Delivery.LastDeliveredSequence, command, route.Room.Slug, command, route.Room.Slug, command, route.Room.Slug)
}
