...
Document uploads are immutable revisions: when the same participant uploads the same filename, Crewfold keeps one logical document and appends navigable history. Filename lookup reads the current revision; an exact document ID reads one historical revision. Same-named uploads from different participants are separate documents. The logical document count should therefore remain stable while one document's revision count grows. Archive obsolete drafts and reports once their useful conclusions have been incorporated into clean canonical documentation. Archiving hides the whole logical document from the active list without deleting its revisions; uploading a new revision under the same publisher and filename automatically makes it active again. Never inspect, edit, delete, or consolidate Crewfold's private SQLite rows or stored blobs directly. Use only the Crewfold CLI for shared-room state.
