...
func (s *Store) applyDomainWorkProposal(ctx context.Context, tx *sql.Tx, proposal domain.DomainWorkProposal, correlationID, now string) ([]domain.DomainWorkProposalEffect, error) {
	...
	for _, item := range proposal.Content.Tasks {
		...
		task := domain.Task{ID: id, WorkspaceID: proposal.WorkspaceID, ProjectID: proposal.ProjectID, ObjectiveID: objective.ID, Title: strings.TrimSpace(item.Title), Description: strings.TrimSpace(item.Description), TaskClass: item.TaskClass, Status: domain.TaskReady, Priority: item.Priority, Budget: item.Budget, Revision: 1, CreatedAt: now, UpdatedAt: now, CreatedBy: localOwnerActorID, UpdatedBy: localOwnerActorID}
		if _, err := tx.ExecContext(ctx, `INSERT INTO tasks(id,workspace_id,project_id,objective_id,title,description,task_class,status,blocked_reason,priority,budget_tokens,budget_cost_cents,budget_time_seconds,revision,created_at,updated_at,created_by,updated_by) VALUES(?,?,?,?,?,NULLIF(?,''),?,'ready',NULL,?,?,?,?,1,?,?,?,?)`, task.ID, task.WorkspaceID, task.ProjectID, task.ObjectiveID, task.Title, task.Description, task.TaskClass, task.Priority, task.Budget.TokenLimit, task.Budget.CostCents, task.Budget.TimeSeconds, now, now, localOwnerActorID, localOwnerActorID); err != nil {
