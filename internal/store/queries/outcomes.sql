...
-- name: ListAcceptedOutcomeAssessmentClaims :many
...
-- name: ListUnassessedOutcomeCommitments :many
...
JOIN outcome_commitment_receipts receipt ON receipt.commitment_id=commitment.id
WHERE commitment.workspace_id=sqlc.arg(workspace_id)
 AND receipt.event_sequence<=sqlc.arg(event_cursor)
 AND (CAST(sqlc.arg(project_id) AS TEXT)='' OR commitment.project_id=sqlc.arg(project_id))
 AND (CAST(sqlc.arg(objective_id) AS TEXT)='' OR commitment.objective_id=sqlc.arg(objective_id))
 AND (CAST(sqlc.arg(task_id) AS TEXT)='' OR commitment.task_id=sqlc.arg(task_id))
...
-- name: ListOpenOutcomeContradictions :many
...
WHERE contradiction.workspace_id=sqlc.arg(workspace_id) AND contradiction.status='open'
 AND contradiction.confirm_event_sequence<=sqlc.arg(event_cursor)
...
-- name: OutcomeContradictionTouchesScopeDecision :one
SELECT EXISTS(
 SELECT 1 FROM knowledge_revisions revision
 JOIN knowledge_items item ON item.id=revision.item_id
 WHERE revision.id IN (sqlc.arg(left_revision_id),sqlc.arg(right_revision_id))
  AND item.workspace_id=sqlc.arg(workspace_id)
  AND (
   (CAST(sqlc.arg(task_id) AS TEXT)<>'' AND (item.task_scope_id IS NULL OR item.task_scope_id=sqlc.arg(task_id)))
   OR (CAST(sqlc.arg(task_id) AS TEXT)='' AND CAST(sqlc.arg(objective_id) AS TEXT)<>'' AND
       (item.task_scope_id IS NULL OR item.task_scope_id IN (SELECT task.id FROM tasks task WHERE task.objective_id=sqlc.arg(objective_id))))
  )
) AS relevant;
...
-- name: GetManagementBriefingClaim :one
SELECT * FROM management_briefing_claims WHERE briefing_id=sqlc.arg(briefing_id) AND claim_id=sqlc.arg(claim_id);

-- name: ListManagementBriefingClaimSources :many
SELECT * FROM management_briefing_claim_sources
WHERE briefing_id=sqlc.arg(briefing_id) AND claim_id=sqlc.arg(claim_id) ORDER BY ordinal;
