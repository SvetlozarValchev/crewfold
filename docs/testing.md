...
| `M22-WAKE-01` | both children reply while their coordinator is still processing the owner turn | the active thread receives no injected wake; each leased wake is durably deferred, then delivered once as a separate `crewfold_delivery` turn after the owner turn completes; restart/retry cannot duplicate the message or its tool receipt |
