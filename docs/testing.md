...
| `M23-PROP-01` | coordinator creates specialists before the objective, then submits a proposal freezing checkout, placements, profiles, tasks, dependencies, and intents; race acceptance with checkout/profile/membership changes | one successful acceptance atomically creates and binds the workstream, places every exact agent, creates the graph/intents, and records every effect; stale/raced/replayed revisions produce either the identical complete result or no effect |
