...
| `M21-AGENT-03` | completion evidence is rejected, so the run enters `review` while its task records `changes_requested`; use the displayed retry, replay it, then use a stale/superseded control | the prior review remains immutable, exact run/task revisions atomically reopen the retained assignment and request one fresh context-bound run, replay returns that run, and stale/superseded attempts create no effect |
...
| `M22-SES-02` | delegation-first conversation names implementation/review/verification roles, receives ambiguous owner assent, and attempts provider-local collaboration or direct source edits | new and resumed threads retain the exact read-only sandbox/current instructions; no source byte changes; temporary helpers are labeled provider-local and cannot substitute for durable children; missing staffing class or run authority is reported exactly |
| `M22-SES-03` | one durable agent accumulates owner, task, mailbox, and tool turns in one Herdr-hosted Codex thread, completes a turn, compacts natively, rotates normally, restarts its provider host, and exercises an injected memory-pressure emergency during a live run | the UI presents one logical agent Session; the run's activity and blocker are navigable in that same timeline; a terminal turn retains the current epoch's observable host; native compaction deliberately recycles the host while preserving the epoch; normal rotation freezes one readable historical epoch and starts one current epoch from a bounded canonical handoff without losing receipts/provenance; another agent's host is unaffected; emergency loss never fabricates a clean handoff or frees uncertain runtime authority |
...
| `M22-WORK-01` | real granted coordinator creates two durable children, sends their exact responsibilities, and submits an implementation → independent verification → knowledge task graph | submission has no task/run effect; owner acceptance atomically creates one workstream, three tasks, two dependencies, and three scheduling intents; only the ready implementation launches, later tasks advance in dependency order, and exact proposal replay is stable |
| `M22-WAKE-01` | children reply while their coordinator is first processing an owner turn and later working an accepted task turn; restart between run launch and delivery | the unrelated owner turn receives no injected wake and each leased wake is durably deferred, then delivered once as a separate `crewfold_delivery` turn; during accepted work, mailbox input steers that exact task turn in the same durable thread; restart reconstructs that route and cannot duplicate the message or its tool receipt |
| `M22-KNOW-01` | the real implementation/verification chain proposes a sourced domain revision | the proposal remains non-current and absent from accepted Domain Home knowledge until the owner accepts its exact revision; acceptance preserves source task/run provenance and survives restart |
| `M22-KNOW-02` | an accepted agent-run revision cites a named Markdown artifact through an applied report and its legacy body contains raw run/artifact IDs | Domain Home resolves the durable agent and source task, presents the report as an openable bounded attachment, and shows readable prose without exposing raw entity IDs in the normal knowledge card; the sealed revision and exact audit identities remain unchanged |
...
M23 consolidates workstream placement, persistent checkout reuse, dependency
output delivery, and logical-agent activity under ADR-0022.

| ID | Adversarial setup | Required observable result |
| --- | --- | --- |
| `M23-WS-01` | create coordination-only and implementation workstreams with absent, read-only, available, moved, and revision-changed primary checkouts; attempt to move a retained workstream | coordination-only omission is explicit; source-mutating work cannot be accepted/scheduled without one current writable primary; the creation binding is revisioned and immutable for M23, so moving work requires closing/reviewing a new graph; no clone/bootstrap path is created |
| `M23-WS-02` | bind two active workstreams to one concrete checkout under exclusive, claimed, and shared policy | both workstreams and the checkout show the exact sharing warning; exclusive writers serialize, claimed writers require disjoint claims, shared writers retain the warning, and sequential reuse remains available |
| `M23-ROOT-01` | open one domain-level agent and two workstream agents in a domain with three attached checkouts | the domain-level conversation can inspect all three through bounded read-only authority; each workstream agent defaults to its own primary checkout; none gains mutation authority from placement or visibility |
| `M23-PROP-01` | coordinator submits one proposal freezing an inert new/existing team hierarchy, checkout, placements, profiles, tasks, dependencies, and intents; inspect before acceptance and race acceptance with checkout/profile/membership/name changes | submission and rejection create zero proposed agents or work; one successful acceptance atomically creates/places the complete team and workstream graph/intents and records every effect; stale/raced/replayed revisions produce either the identical complete result or no effect |
| `M23-CTX-01` | implementation completes with changed paths/checks/handoff/evidence, independent review completes with three structured findings, and remediation requires `handoff_with_evidence` | remediation stays unscheduled until the complete output exists; its packet contains exact predecessor summary, handoff, findings/risks, checks, changed paths, and evidence references; scoped reads return only those referenced artifacts |
| `M23-CTX-02` | omit, corrupt, oversize, retire, or make inaccessible one required predecessor output | successor provider is never launched; blocker names the predecessor, requirement, missing/invalid record, and safe repair; no generic resume action is offered; deterministic repair rebuilds context and relaunches once without changing product intent |
| `M23-AGENT-01` | durable conversation is idle while its attached run starts, works, blocks, resumes correctly, completes, rotates epoch, and restarts Herdr/daemon/browser | selected agent aggregate status follows the consequential run; one ordered timeline renders owner/provider turns, execution commands, changes, messages, blocker, checks, evidence, handoff, and epoch boundaries without claiming shared private reasoning |
| `M23-WARM-01` | register a pre-existing checkout containing dependency/build-cache sentinels, then execute implementation, review, remediation, and verification through separate provider processes | every run uses the exact same canonical path; sentinels and warmed state survive; no clone, install, clean, or bootstrap occurs unless explicitly required by the accepted task/check contract |
| `M23-UI-01` | navigate domain, workstream, agent, shared-checkout warning, missing-input blocker, and repaired successor at desktop and narrow widths | placed team, primary checkout, ready/running/gated tasks, exact gating reasons, unified activity, and safe next actions are readable and keyboard accessible; empty decorative peer tabs and hidden attached work are absent |
...
| `M24-REVIEW-01` | independent review accepts a structured `changes_requested` or `block` assessment in a graph with an explicit remediation successor | the reviewer run/task complete and release their assignment; remediation alone becomes runnable with the exact handoff/evidence; the UI says findings were delivered and names the next task/owner; the rejected-evidence retry is absent |
