...
function ManagedProcesses({ data, apiBase, csrf, mutable, reload }: { data: WorkbenchData; apiBase: string; csrf: string; mutable: boolean; reload: () => Promise<void> }) {
  ...
  const reviewedDefinitions = data.processDefinitions.filter((definition) => definition.status === "active" && !pendingRequests.some((request) => request.definition_id === definition.id && definition.created_by === request.agent_id));

  return <section className="m22-block m24-processes">
    <div className="m22-block-heading"><div><h2>managed local processes</h2><p>Ask a durable agent to run or expose what you need. It inspects the checkout and brings the exact command here for one review.</p></div><button className="m22-command" disabled={!mutable} onClick={() => setCreating((value) => !value)}>{creating ? <X size={13} /> : <Plus size={13} />}{creating ? "close manual setup" : "advanced: define manually"}</button></div>
    {error && <p className="m22-session-error" role="alert">{error}</p>}
    {pendingRequests.length > 0 && <div className="m24-process-requests"><h3>agent process proposals · no effect yet</h3>{pendingRequests.map((request) => {
      ...
      const authored = definition?.created_by === request.agent_id;
      const exactCheckout = definition ? data.checkouts.find((item) => item.id === definition.checkout_id) : null;
      const objective = definition ? data.objectives.find((item) => item.id === definition.workstream_id) : null;
      return <article key={request.id}><div><strong>{agent?.name ?? request.agent_id} proposes {definition?.name ?? request.definition_id}</strong><p>{request.summary}</p>{definition && <><code>{managedProcessCommand(definition)}</code><dl><div><dt>checkout</dt><dd>{exactCheckout?.path ?? definition.checkout_id}</dd></div><div><dt>scope</dt><dd>{objective?.title ?? "domain-wide"}</dd></div><div><dt>working directory</dt><dd>{definition.working_directory}</dd></div><div><dt>network</dt><dd>{definition.network_mode}</dd></div><div><dt>health</dt><dd>{definition.health.type}{definition.health.port ? ` · ${definition.health.host}:${definition.health.port}${definition.health.path ?? ""}` : ""}</dd></div><div><dt>restart</dt><dd>{definition.restart_policy.replaceAll("_", " ")}</dd></div></dl></>}<small>{authored ? `Accepting grants ${agent?.name ?? "this agent"} inspect, logs, start, stop, and restart authority for only this definition revision, then starts one instance. Rejecting retires the draft and starts nothing.` : `Accepting starts one instance from existing definition revision ${request.definition_revision}; rejecting starts nothing.`}</small></div><div><button disabled={!mutable || !!busy} onClick={() => void decideRequest(request, false)}>reject</button><button className="primary" disabled={!mutable || !!busy} onClick={() => void decideRequest(request, true)}>{busy === `request-${request.id}` ? "applying…" : authored ? "accept, grant, and start" : "accept and start"}</button></div></article>;
    })}</div>}
    ...
    {reviewedDefinitions.length ? <div className="m24-process-list">{reviewedDefinitions.map((definition) => {
    ...
    })}</div> : !creating && <p className="m22-empty">No managed process is defined. Ask a durable agent to run or expose what you need; it can inspect the checkout and submit the exact command for one owner review. The manual definition control remains available for advanced recovery and explicit owner setup.</p>}
