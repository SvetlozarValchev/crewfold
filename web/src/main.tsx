...
function WorkbenchView({ data, apiBase, csrf, reload, selectTask, selectRun, mutable }: { data: WorkbenchData; apiBase: string; csrf: string; reload: () => Promise<void>; selectTask: (task: TaskDetail) => void; selectRun: (run: Run) => void; mutable: boolean }) {
  ...
  const attentionRuns = data.agents.map((agent) => latestRunForAgent(data.runs, agent.id)).filter((run): run is Run => Boolean(run) && ["requested", "starting", "active", "blocked", "stopping", "start_failed", "lost"].includes(run.status));
  ...
  return <div className="view-grid workbench-view">
    ...
    <aside className="right-stack">
      ...
      <section className="panel compact-list"><div className="panel-title"><h2>Agents and launch attention</h2><button onClick={() => void reload()} aria-label="Refresh workbench"><RefreshCw size={15} /></button></div>{attentionRuns.length === 0 ? <EmptyState icon={Bot} title="No agent is running" detail="Assigned work is ready for you to inspect and start." /> : attentionRuns.slice(0, 5).map((run) => <button className="list-row" key={run.id} onClick={() => selectRun(run)}><span className="row-icon"><Bot size={16} /></span><span><strong>{data.agents.find((agent) => agent.id === run.agent_id)?.name ?? "Agent"}</strong><small>{run.status === "start_failed" ? run.failure_message ?? "Launch failed; inspect and retry." : data.tasks.find((task) => task.task.id === run.task_id)?.task.title ?? "Assigned work"}</small></span><StatusPill value={run.status} /></button>)}</section>
    </aside>
