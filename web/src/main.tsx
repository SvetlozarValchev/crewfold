...
type DomainWorkProposalTask = { key: string; title: string; description: string; task_class: string; priority: number; budget: Budget; launch_profile_id: string; agent_id: string; agent_membership_revision: number; depends_on: string[]; dependency_delivery: Record<string, "completion" | "handoff" | "handoff_with_evidence"> };
type DomainWorkProposal = { id: string; workspace_id: string; project_id: string; source_agent_id: string; source_thread_id: string; staffing_grant_id: string; staffing_grant_revision: number; summary: string; as_of_event_sequence: number; content: { objective_title: string; objective_budget: Budget; primary_checkout_id: string; primary_checkout_revision: number; reference_checkout_ids: string[]; tasks: DomainWorkProposalTask[] }; content_sha256: string; status: "pending" | "accepted" | "rejected" | "stale"; decision_note?: string; revision: number; created_at: string; updated_at: string; decided_at?: string };
...
function DomainWorkProposalReview({ data, proposal, apiBase, csrf, mutable, reload }: { data: WorkbenchData; proposal: DomainWorkProposal; apiBase: string; csrf: string; mutable: boolean; reload: () => Promise<void> }) {
  ...
  return <article className={`m22-work-proposal ${proposal.status}`}>
    ...
    <dl className="m22-proposal-facts"><div><dt>proposed by</dt><dd>{source?.name ?? proposal.source_agent_id}</dd></div><div><dt>primary checkout</dt><dd>{checkout?.path ?? proposal.content.primary_checkout_id} · r{proposal.content.primary_checkout_revision}</dd></div><div><dt>owner effect</dt><dd>1 workstream · {proposal.content.tasks.length} placed agents · {proposal.content.tasks.length} tasks · {proposal.content.tasks.reduce((count, task) => count + task.depends_on.length, 0)} dependencies · {proposal.content.tasks.length} scheduling intents</dd></div><div><dt>objective budget</dt><dd>{staffingBudgetLabel(proposal.content.objective_budget.token_limit, "tokens")} · {staffingBudgetLabel(proposal.content.objective_budget.time_seconds, "seconds")}</dd></div></dl>
    <ol className="m22-proposal-tasks">{proposal.content.tasks.map((task) => {
      const assignee = data.agents.find((candidate) => candidate.id === task.agent_id);
      return <li key={task.key}><div><strong>{task.title}</strong><small>{task.task_class.replaceAll("-", " ")} · priority {task.priority} · place and assign {assignee?.name ?? task.agent_id} at membership r{task.agent_membership_revision}</small><p>{task.description}</p>{task.depends_on.length > 0 && <span>after {task.depends_on.map((key) => `${taskByKey.get(key)?.title ?? key} (${(task.dependency_delivery[key] ?? "completion").replaceAll("_", " ")})`).join(", ")}</span>}</div><code>{task.key}</code></li>;
    })}</ol>
    <div className="m22-exact-effect"><ShieldCheck size={15} /><span><strong>Exact effect</strong> Accepting atomically binds the workstream to the checkout, places the named durable agents, and creates the tasks, delivery-aware dependencies, and scheduling intents shown above. Eligible tasks then launch in that existing checkout under current policy; acceptance itself does not edit repository files.</span></div>
