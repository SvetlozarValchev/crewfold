...
if [ "$actual" != "$expected" ]
then
  printf 'embedded workbench assets are stale; run ./scripts/build-web.sh\n' >&2
  exit 1
fi
...
if [ "$asset_count" -lt 2 ]
then
  printf 'embedded workbench index does not reference its content-hashed assets\n' >&2
  exit 1
fi

if find web/dist -type l -o -type p -o -type s | grep -q .
then
  printf 'embedded workbench tree contains an unsafe non-regular entry\n' >&2
  exit 1
fi
if find web/dist -type f -name '*.map' | grep -q .
then
  printf 'embedded production workbench contains source maps\n' >&2
  exit 1
fi
total_bytes=$(find web/dist -type f -printf '%s\n' | awk '{ total += $1 } END { print total + 0 }')
if [ "$total_bytes" -gt 5242880 ]
then
  printf 'embedded workbench exceeds the frozen 5 MiB asset limit: %s bytes\n' "$total_bytes" >&2
  exit 1
fi

printf 'Embedded workbench assets: PASS (%s bytes)\n' "$total_bytes"
